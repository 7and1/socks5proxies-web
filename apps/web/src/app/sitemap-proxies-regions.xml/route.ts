...
export async function GET(request: Request) {
  ...
  try {
    const response = await getFacetPage("regions", {
      limit: PAGE_SIZE,
      offset,
    });
    data = response.data ?? [];
  ...
  const locations = data
    .filter((region) => region.count >= MIN_PROXIES_FOR_PAGE)
    .map((region) => {
      const [country, ...rest] = region.key.split("|");
      if (!country || rest.length === 0) return "";
      const name = rest.join("|");
      return `${BASE_URL}/free-proxy-list/region/${country.toLowerCase()}/${encodeURIComponent(
        name,
      )}`;
    })
